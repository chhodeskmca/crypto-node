...
exports.updatePayoutSettings = async (adminId,minimumBalance) => {
    console.log("adminId, minimumBalance", adminId, minimumBalance)
  try {
    ...
    // Update users created by this admin
    await User.updateMany(
      { createdBy: new ObjectId(adminId) },
      { $set: { minPayoutAmount: minimumBalance } },
    //   { session },
    );
    ...
    throw new Error("Error updating payout settings",error);
  }
};
...
exports.createPayoutRequestFromBalance = async () => {
  try {
    ...
    const adminMinimumBalance = payoutSetting
      ? payoutSetting.minimumBalance
      : 0;

    const balances = await Balance.find({ kaspa: { $gt: adminMinimumBalance } })
      .lean()
      .exec();
    ...
    for (const balance of balances) {
      ...
      const validRequest = await User.findOne({
        _id: userId,
        minPayoutAmount: { $gte: adminMinimumBalance },
      })
        .lean()
        .exec();
